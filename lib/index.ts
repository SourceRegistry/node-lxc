...
//springdalelinux;9;amd64;default;20250508_06:38;/images/springdalelinux/9/amd64/default/20250508_06:38/
export type Image = {
...
const HelperFunctions = {
    /**
     * Parses a line of text containing image information and returns an Image object
     * Format: distribution;release;architecture;variant;build_date;url/
     * Example: springdalelinux;9;amd64;default;20250508_06:38;/images/springdalelinux/9/amd64/default/20250508_06:38/
     */
    parseImageLine(line: string): Image | null {
        const regex = /^([^;]+);([^;]+);([^;]+);([^;]+);(\d{8}_\d{2}:\d{2});(\/images\/[^;]+)\/$/;
        const match = line.match(regex);

        if (!match) return null;

        // Extract matched groups
        const [, distribution, release, architecture, variant, dateStr, url] = match;

        // Parse the date string (format: YYYYMMDD_HH:MM)
        const year = parseInt(dateStr.substring(0, 4));
        const month = parseInt(dateStr.substring(4, 6)) - 1; // JS months are 0-indexed
        const day = parseInt(dateStr.substring(6, 8));
        const hour = parseInt(dateStr.substring(9, 11));
        const minute = parseInt(dateStr.substring(12, 14));
        const build_date = new Date(year, month, day, hour, minute);
    ...
    /**
     * Parses multiple lines of image data
     */
    parseImageData(data: string): Image[] {
        return data
            .split('\n')
            .filter(line => line.trim() !== '')
            .map(HelperFunctions.parseImageLine)
            .filter((image): image is Image => image !== null);
    }
}
export const Images = {
    async List(repository: string = "https://images.linuxcontainers.org//meta/1.0/index-user"): Promise<Image[]> {
        const response = await fetch(repository);
        if (!response.ok) throw new Error(`Failt to get image list from '${repository}'`, {cause: response});
        return response.text().then(HelperFunctions.parseImageData, () => []);
    },

    async Download(image: Image){
        if(!image.url) throw new Error('Unable download image without url')
